        ...
        public void onGuildMessageReceived(GuildMessageReceivedEvent event) {
            ...
            if (arg.length == 8 && (arg[0] + arg[1]).equalsIgnoreCase(Bot.prefix + "make")) {
                ...
                if (check) {
                    ...
                    event.getChannel().sendMessage("Hey " + event.getMember().getAsMention() + ", recheck your syntax").queue();
                }
            }
            if (arg.length == 2 && (arg[0] + arg[1]).equalsIgnoreCase(Bot.prefix + "makeSolved")) {
                ...
                String s = Update.getCubeSQL(event.getMember().getId()).replace("null", "") ; ;
                String store  = Update.getStoreSQL(event.getMember().getId()) ;
                if (!s.equals("Error")) {
                    cube = new Cubot(s.split(" "));
                    EmbedBuilder info = new EmbedBuilder();
                    info.setColor(0Xa80d2c);
                    String name;
                    boolean solvedatfirst = true;
                    System.out.println((Update.getsSolvedSQL(event.getMember().getId())));
                    if (solvedatfirst && store.length() < 20) {
                        String s1 = cube.reversealg(store, false);
                        name = cube.stringalg(s1);
                    } else {
                        name = cube.solve();
                    }
                    Update.setCubeSQL(event.getMember().getId(), "" , cube.toarr()) ;
                ...
                if (!s.equals("Error")) {
                    ...
                    if (check) {
                        ...
                        event.getChannel().sendMessage("Re-enter with valid sytanx" + event.getMember().getAsMention()).queue();
                    }
