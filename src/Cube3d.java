public class Cube3d {
    ...
    public Cube3d(String[] temp) throws IllegalArgumentException {
    ...
    public String toarr() {
        ...
        for (int i = 0 ; i < store1.length ; i++ ) {
            if ( i >  2 && i < 6 ) {
                ...
                charStore[5] += store1[i].substring(5,8);
            }
            else {
                charStore[4] += store1[i].substring(5, 8);
            }
        }
    ...
    public boolean solved(Cube3d other) {
        for (int j = 0; j < cube.length; j++) {
            for (int i = 0; i < cube.length; i++) {
                for (int k = 0; k < cube.length; k++) {
                    if (cube[i][j][k][0] == null && cube[i][j][k][2] == null) {
